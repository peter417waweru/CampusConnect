...
def staff_login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                # Redirect to appropriate staff dashboard or page after successful login
                return redirect('uber:create_driver')  # Replace with your staff dashboard URL name
    else:
...
@login_required
@staff_member_required  # Or permission check decorator
def driver_list(request):
    drivers = User.objects.filter(is_staff=False)  # Filter for non-staff users (drivers)
...
def uber_driver_login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = authenticate(username=username, password=password)
            if user is not None:
                # Check if user has a corresponding Driver model instance (registered driver)
                try:
                    driver = Driver.objects.get(user=user)  # Attempt to get the Driver object
                    login(request, user)
                    return redirect('uber:driver_dashboard')  # Redirect to driver dashboard
                # You can add an error message here if needed
                except Driver.DoesNotExist:
                    error(request, 'You are not a registered Uber Driver.')  # Add error message
                    pass
    else:
        form = AuthenticationForm()
...
def add_vehicle(request):
    ...
    try:
        ...
        # Optionally, redirect to an error page or create a new driver profile
...
def view_requests(request):
    requests = UberRequest.objects.filter(status='requested').order_by('-created_at')
    return render(request, 'uber/drivers/view_requests.html', {'requests': requests})
