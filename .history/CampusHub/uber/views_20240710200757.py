...
def track_uber(request, pk):
    ...
    if uber_request.status == 'accepted':
        message = f"Your Uber request has been confirmed. Estimated arrival time: {uber_request.estimated_arrival_time}"
    else:
        message = "Your Uber request is still pending confirmation."
  
    return render(request, 'uber/students/track_uber.html', {'uber_request': uber_request, 'message':message})
